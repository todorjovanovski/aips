...
public class ArrayMeanValue {
    ...
    public static int brojDoProsek(Array<Integer> arr) {
        ...
        double minDistance = Math.abs(arr.get(0) - average);
        for(int i=1; i < arr.getSize(); i++) {
            int curr = arr.get(i);
            double distance = Math.abs(curr - average);
            if(distance < minDistance || (distance == minDistance && curr < closest)) {

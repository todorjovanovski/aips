...
public class Lozinki {
    public static void main (String[] args) throws IOException {
        ...
        CBHT<String, String> logs = new CBHT<>((int) (N/0.75));

        for(int i=1;i<=N;i++){
            String imelozinka = br.readLine();
            String[] pom = imelozinka.split(" ");
            logs.insert(pom[0], pom[1]);
        }

        while (true) {
            String credentials = br.readLine();
            if(credentials.equals("KRAJ")) break;
            String[] parts = credentials.split("\\s+");
            String username = parts[0];
            String password = parts[1];

            if(logs.search(username) != null && logs.search(username).element.value.equals(password)) {
